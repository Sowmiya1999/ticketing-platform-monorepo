...
export default async function BookingSuccess({
  ...
  const currentPrice = event?.currentPrice ?? booking.snapshotPrice;

  const breakdown = booking.priceBreakdown.breakdown as PriceBreakdown;
    ...
    <div className="bg-white p-6 rounded-lg shadow-md max-w-xl mx-auto mt-8">
      ...
      <div className="mt-6 border-t pt-4">
        ...
        <div className="space-y-1 text-sm text-gray-700">
          ...
          <p>
            Time Factor: +{breakdown.timePercentage}% (
            ₹{breakdown.timeAmount.toFixed(2)})
          </p>
          <p>
            Demand Factor: +{breakdown.demandPercentage}% (
            ₹{breakdown.demandAmount.toFixed(2)})
          </p>
          <p>
            Inventory Factor: +{breakdown.inventoryPercentage}% (
            ₹{breakdown.inventoryAmount.toFixed(2)})
          </p>
          <p className="font-semibold mt-2">
            Final Price Paid: ₹{booking.pricePaid}
          </p>
        </div>
      </div>
    </div>
