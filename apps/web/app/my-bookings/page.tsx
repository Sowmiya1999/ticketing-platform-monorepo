        ...
        <div className="space-y-4">
          {bookings.map((b) => {
            ...
            if (b.currentPrice && b.pricePaid) {
              ...
              if (diff > 0) {
                message = `You saved ₹${diff}`;
                messageColor = 'text-green-600';
              } else if (diff < 0) {
                message = `You lost ₹${Math.abs(diff)}`;
            ...
            return (
              <div key={b.id} className="p-[2px] rounded-lg bg-gradient-to-r from-blue-600 to-orange-400">
                <div className="bg-white rounded-lg p-4 flex justify-between items-center">
                  <div>
                    ...
                    <div className="text-sm text-gray-600">Booked Tickets: {b.quantity}</div>
                  </div>
                  <div className="text-right">
                    <div className="font-medium text-gray-900">Paid: ₹{b.pricePaid}</div>
                    {b.currentPrice && (
                      <div className="text-sm text-gray-500">Current: ₹{b.currentPrice * b.quantity}</div>
                    )}
                    {message && <div className={`text-sm font-medium ${messageColor}`}>{message}</div>}
                  </div>
                </div>
              </div>
            );
